package kim.bifrost.rain.bilibili.widget.parser;
...
/**
 * DanmakuFlameMaster的Sample Parser
 * 我怀疑就是那时BiliBili客户端的源码
 * 正好用来解析b站的弹幕
 *
 * 那时还没有弹幕屏蔽等级
 * 现在有了，加上适配
...
public class BiliDanmakuParser extends BaseDanmakuParser {
    ...
    public class XmlContentHandler extends DefaultHandler {
        ...
        public void startDocument() {
            result = new Danmakus(IDanmakus.ST_BY_TIME, false, mContext.getBaseComparator());
        }
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) {
            ...
            if (tagName.equals("d")) {
                ...
                if (values.length > 0) {
                    ...
                    int level = parseInteger(values[8]);
                    // 弹幕等级大于等于屏蔽等级则显示弹幕
                    if (level >= App.Companion.getDanmakuShieldLevel()) {
                        item = mContext.mDanmakuFactory.createDanmaku(type, mContext);
                        if (item != null) {
                            item.setTime(time);
                            item.textSize = textSize * (mDispDensity - 1.2f);
