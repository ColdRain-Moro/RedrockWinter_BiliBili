...
class VideoViewModel(private val data: SimpleVideoInfo?) : ViewModel() {
    suspend fun getVideoPlayData(bvid: String, cid: Int) = ApiService.getVideo(avid = null, bvid = bvid, cid = cid)
    suspend fun getVideoInfo() = ApiService.getVideoInfo(null, data!!.bvid).data
}